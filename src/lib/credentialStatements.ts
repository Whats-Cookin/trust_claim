...
function generateOpenBadgesStatement(credential: CredentialData): string {
  ...
  if (achievement) {
    ...
    const issuerName =
      typeof credential.issuer === 'string' ? credential.issuer : credential.issuer?.name || 'the issuer'
 ...
 */
export function generateCredentialStatement(credential: CredentialData, schema?: string): string {
