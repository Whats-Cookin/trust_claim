...
import { Edit, Delete, Info, AccountBalanceWallet, Key } from '@mui/icons-material'
import {
  getUserIdentity,
  setCustomDid,
  clearCustomDid,
...
export const IdentityManager: React.FC<IdentityManagerProps> = ({ open, onClose, onIdentityChange }) => {
      ...
      <DialogContent>
        <Box sx={{ mb: 3 }}>
          ...
          >
            ...
            {currentIdentity.did && localStorage.getItem('userDid') && (
              <IconButton size='small' onClick={handleClearCustomDid} sx={{ ml: 1 }} color='error'>
        ...
        <Box sx={{ mb: 2 }}>
          ...
          <Typography variant='caption' color='text.secondary' paragraph>
            If you have a DID from another system (did:key, did:web, did:ion, etc.), you can use it here.
          </Typography>
          ...
          <Button variant='contained' onClick={handleSetCustomDid} disabled={!customDidInput} startIcon={<Edit />}>
            Set Custom DID
          </Button>
        </Box>
        ...
        <Alert severity='info' sx={{ mt: 2 }}>
          <Typography variant='caption'>
            Your DID identifies you across all your claims. Using a custom DID allows you to maintain the same identity
...
export const IdentityButton: React.FC = () => {
  ...
  return (
    <>
      <Chip icon={<Key />} label={identity} onClick={() => setOpen(true)} variant='outlined' size='small' />

      <IdentityManager open={open} onClose={() => setOpen(false)} onIdentityChange={handleIdentityChange} />
    </>
  )
}
